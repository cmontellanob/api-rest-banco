...
/**
 * REST controller for managing {@link uasb.edu.domain.Cliente}.
 */
@RestController
@RequestMapping("/api")
public class ClienteResource {
    ...
    public ResponseEntity<Cliente> updateCliente(@PathVariable Long id,@RequestBody Cliente cliente) throws URISyntaxException {
    ...
    /**
     * PATCH  /clientes/:id : Updates an existing area.
     *
     * @param cliente the area to update
    ...
    public ResponseEntity<Cliente> partialUpdateCliente(@PathVariable Long id, @RequestBody Cliente cliente)
        ...
        cliente.setId(id);
        Cliente result = clienteService.partialSave(cliente);
        return ResponseEntity.ok()
                .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, cliente.getId().toString()))
                .body(result);
    }
